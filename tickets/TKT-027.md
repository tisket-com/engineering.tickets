...
**Decision:** Use org-prefixed sequential IDs: `[THD-ACME-1]`

**Format:** `[THD-<ORG>-<SEQ>]`
- `THD` - Fixed prefix identifying Tisket threads
- `<ORG>` - Organization slug (uppercase)
- `<SEQ>` - Sequential number within the inbox (no zero-padding)

**Examples:**
- `[THD-ACME-1]` - First thread for Acme
- `[THD-ACME-47]` - 47th thread for Acme
- `[THD-ACME-1000]` - 1000th thread for Acme
- `[THD-GLOBEX-12]` - 12th thread for Globex

**Why no zero-padding:**
- Scales naturally: `1` → `999` → `1000` → `12345`
- No awkward transitions when crossing digit boundaries
- Same approach used by Jira (`PROJ-1`, `PROJ-1000`)
...
2. Postmark webhook → Tisket creates thread THD-ACME-1
3. Tisket sends auto-acknowledgment via Postmark:
   - To: client
   - From: `support.acme@tisket.com`
   - Subject: `[THD-ACME-1] Re: Login issue`
   - Body: "Your request has been received. Reference: THD-ACME-1"
4. Client replies → subject contains `[THD-ACME-1]` → matched to thread
5. Staff replies (with inbox CC'd) → subject contains `[THD-ACME-1]` → matched to thread
...
> "Your request has been received and we'll respond shortly. Reference: THD-ACME-1"
...
CREATE TABLE threads (
  id            TEXT PRIMARY KEY,   -- THD-ACME-1
...
```
1. Parse thread ID from subject: [THD-ACME-1]
...
- Thread ID generation (`THD-ORG-SEQ`, no zero-padding)

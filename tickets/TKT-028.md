...
**Replace SSE with ref polling.** Poll the `heads` endpoint every 2-5 seconds. This is cheap (MCP reads local `.git/refs/heads/main` files - microseconds) and effectively real-time.

### Change Detection
Add a `heads` tRPC endpoint that returns current HEAD sha for each repo from MCP server's local clones. Client polls every 2-5 seconds, compares to browser git HEADs, and triggers a `git fetch` only for repos where HEAD differs.
...
function useHeadsPolling(repoSlugs: string[]) {
  // Poll `heads` endpoint every 2-5 seconds
...
- No SSE - just simple ref polling every few seconds
